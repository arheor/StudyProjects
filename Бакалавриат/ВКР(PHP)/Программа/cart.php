...
if ($_SESSION['order_delivery'] == "Транспортной компанией") $chck2 = "checked";
...
<li>
<input type="radio" name="order_delivery" class="order_delivery" id="order_delivery2" value="Транспортной компанией" '.$chck2.' />
<label class="label_delivery" for="order_delivery2">Транспортной компанией</label>
</li>
<li>
...
<li><label for="order_phone"><span>*</span>Телефон</label><input type="text" name="order_phone" id="order_phone" value="'.$_SESSION["order_phone"].'" /><span class="order_span_style" >Пример: 8 800 555 35 35</span></li>
<li><label class="order_label_style" for="order_address"><span>*</span>Адрес<br /> доставки</label><input type="text" name="order_address" id="order_address" value="'.$_SESSION["order_address"].'" /><span>Пример: г. Краснодар,<br /> ул Красная д 18, кв 58</span></li>
';
}
echo '
...
';    
  ...
  //параметры оплаты
  $mrh_login = "arheor";
  $mrh_pass1 = "Ntcngfhjkm1";
  $inv_id = 678678;
  $inv_desc = "Оплата заказа";
  $out_summ = $_SESSION["cart_price"];
  $IsTest = 1;
  $crc = md5("$mrh_login:$out_summ:$inv_id:$mrh_pass1");
 echo '
<h2 class="itog-price" align="right">Итого: <strong>'.$itogpricecart.'</strong> руб</h2>
  <center><html><script language=JavaScript src="https://auth.robokassa.ru/Merchant/PaymentForm/FormV.js?MerchantLogin='.$mrh_login.'&OutSum='.$out_summ.'&InvoiceID='.$inv_id.'&Description='.$inv_desc.'&SignatureValue='.$crc.'&IsTest='.$IsTest.'"></script></html></center>
